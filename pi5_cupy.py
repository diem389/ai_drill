...
def estimate_points_num_in_quarter_circle_cuda(sample_num):
    cp.random.seed()

    xs = cp.random.uniform(0, 1, sample_num)
    ys = cp.random.uniform(0, 1, sample_num)
    is_in_circle = (xs * xs + ys * ys) <= 1
    return cp.sum(is_in_circle)
...
if __name__ == '__main__':
    ...
    results = pool.map(estimate_points_num_in_quarter_circle_cuda, map_inputs)
    pool.close()
